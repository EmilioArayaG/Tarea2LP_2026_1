    ...
    for(int i = 0; i < 3; i++) {
        ...
        if(cx >= 0 && cx < j->t->W && cy >= 0 && cy < j->t->H) {
            ...
            if(c_back->pieza && c_back->pieza->tipo != 'R') {
                c_back->pieza->hp -= 1;
                if(c_back->pieza->hp <= 0) { free(c_back->pieza); c_back->pieza = NULL; }
            }
        }
    }
    return true;
...
bool especial(struct Juego *j, int dx, int dy) {
    if(!j->jugador) return false;
    int b_x = j->jugador->x, b_y = j->jugador->y;
